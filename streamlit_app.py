    ...
    if st.button("Classify", type="primary"):
        if not log_text.strip():
            ...
            with st.spinner("Classifying..."):
                try:
                    resp = httpx.post(f"{BACKEND_URL}/classify", json={"text": log_text}, timeout=30)
                    if resp.status_code == 200:
                        ...
                        col_c.metric("Confidence",  f"{confidence*100:.1f}%" if confidence is not None else "N/A (LLM)")
                    else:
                        st.error(f"API error: {resp.text}")
                except httpx.ConnectError:
                    st.error("Cannot reach the backend. Check Railway deployment.")
    ...
    if st.button("Classify All", type="primary"):
        ...
        if not lines:
            ...
            with st.spinner(f"Classifying {len(lines)} messages..."):
                try:
                    resp = httpx.post(f"{BACKEND_URL}/classify/batch", json=lines, timeout=60)
                    if resp.status_code == 200:
                        results = resp.json()
                        rows = [
                            {
...
with tab3:
    ...
    with col_stats:
        ...
        if st.button("Refresh Stats"):
            try:
                resp = httpx.get(f"{BACKEND_URL}/stats", timeout=10)
                if resp.status_code == 200:
                    stats   = resp.json()
    ...
    with col_train:
        ...
        st.caption("Uses `data/sample_logs.csv` on the Railway server (33 rows, 11 per class).")
        if st.button("Train on Sample Data", type="primary"):
            with st.spinner("Training BERT + Logistic Regression on Railway (~2 min)..."):
                try:
                    resp = httpx.post(f"{BACKEND_URL}/train", json={}, timeout=300)
                    if resp.status_code == 200:
